rasterize_shapefile <- function(shp, out_rst, 
  ...
  # if(!file.exists(file.path(dir_rst, out_rst))) {
    ...
    rastered <- raster::raster(pixellate(pspSl, eps=1000, xy = list(y = 2848, x = 4548), 
                                         w = c(xl = -2032092, xu = 2515908, yl = -2116850, yu = 731150), DivideByPixelArea = TRUE))
    crs(rastered) <- "+proj=laea +lat_0=45 +lon_0=-100 +x_0=0 +y_0=0 +a=6370997 +b=6370997 +units=m +no_defs"
    
    rastered <- projectRaster(from = rastered, to = ras_template)
...
if (!file.exists(file.path(transportation_density_dir, "railroad_distance.tif"))) {
  ...
  tmp <- rasterize_shapefile(shp = railroads, out_rst = 'railroads.tif')
